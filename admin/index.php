<?php
error_reporting(E_ERROR); // CHANGE- AUTHOR: Hritik Vijay DATE: 1-May-2014
$page = "index";
include "header.php";
...
if($view == "approved") {
  ...
  $places = mysql_query("SELECT * FROM places ORDER BY id DESC LIMIT $page_start, $items_per_page");
  $total = $total_all;
}
if($search != "") {
...
// CHANGE- AUTHOR: Hritik Vijay DATE: 1-May-2014
              $skills=preg_replace('/_+/', ' ', $place['skills']);
              echo "
              <a class='btn btn-small btn-danger' href='index.php?task=delete&place_id=$place[id]&view=$view&search=$search&p=$p'>Delete</a>
            </div>
            <div class='place_info'>             
              <a href='http://$place[uri]' target='_blank' onmouseover='document.getElementById(\"detailsOfId:".$place[id]."\").style.cssText=\"display:block;border-style:solid;border-color:green;border-width:1px;\";document.getElementById(\"detailsOfId:".$place[id]."\").focus();' onmouseout='document.getElementById(\"detailsOfId:".$place[id]."\").style.cssText=\"display:none;\";' >
               $place[owner_name]
                <span class='url'>
                  $place[uri]
                </span>
              ...
              <div id='detailsOfId:".$place[id]."' style='display:none' >
              Company: $place[title]<br/>
              Email: <a href='mailto:$place[owner_email]'>$place[owner_email]</a><br/>
              Skills: $skills</div>
            </div>
          </li>
        ";
