        ...
        <?php
          ...
          foreach($types as $type) {
            ...
            */
            ...
            $types=array(
                array('Hacking','Hacker'), // First index is of coloumn name and second is of Display name
                array('Blogging','Blogger'),
                array('Programming','Programmer'),
                array("Graphics_Designing","Graphics Designer")
              );
         
          $marker_id = 0;
          foreach($types as $type) {
            $places = mysql_query("SELECT * FROM places WHERE approved='1' AND (skills LIKE '$type[0],%' OR skills LIKE '%,$type[0]' OR skills LIKE '%,$type[0],%' OR skills LIKE '$type[0]') ORDER BY title");
            $places_total = mysql_num_rows($places);


			 while($place = mysql_fetch_assoc($places)) {
			  $place['skills']=preg_replace('/_+/', ' ', $place['skills']);
              ...
              echo "markers.push(['".$place['owner_name']."', '".$place['type']."', '".$place['lat']."', '".$place['lng']."', '".$place['description']."', '".$place['uri']."', '".$place['address']."','".$place['skills']."']);
        ...
        jQuery.each(markers, function(i, val) {
          ...
          google.maps.event.addListener(marker, 'click', function () {
            infowindow.setContent(
              ...
              + "<div class='marker_skills'><small>Skills: "+val[7]+"</small></div>"
...
// CHANGE- AUTHOR: Hritik Vijay DATE: 1-May-2014
            $types=array(
                array('Hacking','Hacker'), // First index is of coloumn name and second is of Display name
                array('Blogging','Blogger'),
                array('Programming','Programmer'),
          ...
          foreach($types as $type) {
            if($type[0] != "event") {
              ...
              $markers = mysql_query("SELECT * FROM places WHERE approved='1' AND (skills LIKE '$type[0],%' OR skills LIKE '%,$type[0]' OR skills LIKE '%,$type[0],%' OR skills LIKE '$type[0]') ORDER BY title");
            } else {
            ...
            while($marker = mysql_fetch_assoc($markers)) {
              echo "
                  <li class='".$marker['type']."'>
                    <a href='#' onMouseOver=\"markerListMouseOver('".$marker_id."')\" onMouseOut=\"markerListMouseOut('".$marker_id."')\" onClick=\"goToMarker('".$marker_id."');\">".$marker['owner_name']."</a>
                  </li>
              ";
              $marker_id++;
            }
    ...
    if(event.which === 27) {
        $('#modal_add').fadeOut(600);
           ...
           <div class="control-group">
              ...
              <div class="controls">
                <input type="text" class="input-xlarge" name="title" id="add_title" maxlength="100" autocomplete="off" value="Not Yet Employed">
              </div>
            </div>
          
          <!--  <div class="control-group">
              <label class="control-label" for="input01">Skilled in</label>
              <div class="controls">
...
$skill_options=array("Hacking","Blogging","Programming","Graphics Designing");
for($i=0;$i<sizeof($skill_options);$i++){
?>
<input type='checkbox' name='skills[]' class='skills' value='<?=$skill_options[$i]?>' id='<?=$i?>'> <label style='display:inline;margin-right:1em' for='<?=$i?>'><?=$skill_options[$i]?> </label><!--Added a few space -->
<?
}
?>
  </div>
  ...
  return selchbox;
      ...
      $("#modal_addform").submit(function(event) {
        ...
        // CHANGE- AUTHOR: Hritik Vijay DATE: 1-May-2014
        var $form = $( this )
          //START Fetching Checkboxes
        
var selchb = getSelectedChbox(document.getElementById('modal_addform'));     // gets the array returned by getSelectedChbox()
          //END
